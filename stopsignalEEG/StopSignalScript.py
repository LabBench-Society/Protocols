...
class StopSignalTask:
    ...
    def Go(self):
        ...
        if self.signal == 0:
            self.display.Display(self.images.Left, True)
        else:
            self.display.Display(self.images.Right, True)                      
...
class TaskFeedback:
    ...
    def GoFeedback(self, answer, time):
        triggerGenerator = self.triggerGenerator
        Triggers = self.triggerTlk
        code = 2 if answer else 4
        triggerGenerator.GenerateTriggerSequence(Triggers.StartTrigger.Response01, 
                                                 Triggers.Sequence()
                                                        .Add(Triggers.Trigger(10).Stimulus().Digital().Code(code)))

        if answer:
            self.display.Display(self.images.Correct, True)
        else:
            self.display.Display(self.images.Wrong, True)
