...
def Total(tc):
    return Stress(tc) + Anxiety(tc) + Depression(tc)

def Complete(tc):
    tc.DASS.Annotations.SetInteger('Stress', Stress(tc))
    tc.DASS.Annotations.SetInteger('Anxiety', Anxiety(tc))
    tc.DASS.Annotations.SetInteger('Depression', Depression(tc))
    tc.DASS.Annotations.SetInteger('Total', Total(tc))
    return True